...
import { wrapHtmlForPdf, escapeHtml } from "../utils/helpers";
...
const buildAuditPageHtml = (student) => {
  ...
  return `
    <div style="display: block; width: 100%; font-family: 'Helvetica', 'Arial', sans-serif; color: #000; line-height: 1.5 !important; background-color: white;">
...
const renderElementAsFullPageImage = async (doc, element, options = {}) => {
  const { margin = 30, shrink = 0.92, scale = 2 } = options;
      ...
      pdf.html(contractDiv.querySelector(".pdf-container"), {
        callback: async (pdfCriado) => {
          try {
            ...
            await renderElementAsFullPageImage(pdfCriado, auditDiv.querySelector(".pdf-container"));
