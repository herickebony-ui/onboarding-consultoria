...
const SignaturePad = ({ onSave, onClear }) => {
  ...
  const finishStroke = (e) => {
    ...
    const canvas = canvasRef.current;
    const ctx = ctxRef.current;
    try { if (canvas && e.pointerId) canvas.releasePointerCapture(e.pointerId); } catch (err) {}
  ...
  return (
    ...
    >
      <canvas
        ...
        style={{ touchAction: "none", display: "block", width: "100%", height: "250px", cursor: "crosshair" }}
      />
      <div className="bg-gray-50 p-2 border-t border-gray-200 flex justify-between items-center px-4">
        <span className="text-[10px] text-gray-400 uppercase font-bold pointer-events-none select-none">Assine no espaço acima</span>
        <button onClick={clear} type="button" className="text-xs text-red-600 font-bold hover:bg-red-50 px-3 py-1.5 rounded transition-colors uppercase tracking-wide select-none">Limpar</button>
      </div>
    </div>
  );
};
