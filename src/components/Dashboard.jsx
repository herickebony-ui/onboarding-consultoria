...
import { collection, doc, updateDoc, setDoc, getDocs, deleteDoc, addDoc, query, where, orderBy } from "firebase/firestore";
import { Copy, Users, FileText, Settings, Plus, Trash2, Edit, Save, X, Search, FileSignature, Palette, CheckCircle, Loader, Share2, ArrowLeft, LogOut, Wallet, History, Link as LinkIcon, ChevronRight } from 'lucide-react';
...
const Dashboard = ({
  ...
  const openApproveModal = (student) => {
    ...
    setSelectedPlanForStudent(student.planId || "");
    setSelectedTemplateId(student.templateId || "");
    setAdminFieldValues({});

    // Se o aluno já tinha plano antes, marcamos como TRUE, senão FALSE (padrão)
    setIncludeOnboarding(!!student.planId);
    setApprovalStep(1);
  ...
  const handleGenerateDraft = () => {
    // Validação Inteligente: Só exige Plano se o toggle estiver LIGADO
  ...
  const handleQuickRegister = async () => {
    // Validação: Só Nome e WhatsApp são obrigatórios
    if (!newStudentName || !newStudentPhone) return alert("Por favor, preencha pelo menos o Nome e o WhatsApp.");

    const data = {
      name: newStudentName,
      phone: newStudentPhone.replace(/\D/g, ''),
      ...extraData, // Salva CPF, RG, Endereço... se tiverem sido preenchidos

      status: 'student_only', // Status inicial: Sem Contrato
      createdAt: new Date().toISOString(),

      // Campos de sistema (iniciam vazios)
      planId: null,
      contractText: "",
      pendingFields: [],
      templateId: null
    ...
    try {
      ...
      // Sucesso: Limpa tudo e fecha o modal
      alert("✅ Aluno cadastrado com sucesso!");
      setIsInviting(false);
      setNewStudentName("");
      setNewStudentPhone("");
      setExtraData({ cpf: '', rg: '', email: '', address: '', birthDate: '', profession: '' });
      setApprovalStep(1); // Reseta para o padrão
    } catch (e) {
      console.error(e);
  ...
  const handleGenerateHistoryPDF = async (contract) => {
    ...
    // Chama a função global de gerar PDF (que está no utils.js ou importada)
  ...
  const handleFinalizeInvite = async () => {
    ...
    try {
      // A. Define ID do Aluno (Existente ou Novo)
      let studentId = editingStudentId;

      // Se for aluno novo, cria o documento dele primeiro
      if (!studentId) {
        const newStudentRef = await addDoc(collection(db, "students"), {
          ...
          ...extraData,
          createdAt: timestamp,
          status: 'student_only' // Status provisório
        });
        studentId = newStudentRef.id;
      }

      // B. CRIA O CONTRATO NA COLEÇÃO SEPARADA (Histórico)
      const newContractRef = await addDoc(collection(db, "contracts"), {
        studentId: studentId,
        contractText: draftContract, // O texto aprovado
        templateId: selectedTemplateId,
        status: "waiting_sign",
        createdAt: timestamp,
        // Salva dados do momento da geração (snapshot)
        studentSnapshot: { name: newStudentName, cpf: extraData.cpf, ...extraData }
      });

      // C. ATUALIZA O ALUNO (Aponta para o contrato novo)
      const updateData = {
        name: newStudentName,
        phone: newStudentPhone.replace(/\D/g, ''),
        ...extraData,

        // Lógica do Toggle: Se desligado, planId vira null
        planId: includeOnboarding ? selectedPlanForStudent : null,

        // Vínculo Mágico: O aluno agora "olha" para este contrato novo
        latestContractId: newContractRef.id,
        latestContractStatus: "waiting_sign",

        pendingFields: studentFields,
        status: 'waiting_sign', // Muda status para "Aguardando"
        lastUpdate: timestamp
      };

      await updateDoc(doc(db, "students", studentId), updateData);

      alert("✅ Contrato gerado com sucesso! O link já está disponível para o aluno.");
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-red-50 text-red-600 rounded-lg font-bold border border-red-100 hover:bg-red-100 transition-colors shadow-sm"
          >
          ...
          <div className="bg-white p-1 rounded-xl border border-gray-200 flex shadow-sm">
            ...
            <button onClick={() => setActiveTab('financial')} className={`px-6 py-2 rounded-lg text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'financial' ? 'bg-black text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'}`}>
        ...
        {activeTab === 'financial' && (
          <div className="animate-in fade-in duration-300">
            <FinancialModule students={students} />
          </div>
        )}
        {activeTab === 'students' && (
          <div className="animate-in fade-in duration-300">
              ...
              <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                <div className="bg-white rounded-xl shadow-2xl w-full max-w-6xl h-[90vh] flex flex-col overflow-hidden animate-in zoom-in duration-200">
                  ...
                  <div className="flex-1 flex overflow-hidden">
                    ...
                    {approvalStep === 0 && (
                      <div className="w-full h-full overflow-y-auto bg-gray-50 p-6 flex flex-col items-center justify-center">
                        <div className="max-w-4xl w-full grid grid-cols-1 md:grid-cols-2 gap-8">
                          {/* OPÇÃO A: CADASTRO MANUAL (COMPLETO) */}
                          <div className="bg-white p-6 rounded-2xl border-2 border-blue-100 shadow-sm hover:border-blue-500 transition-all group flex flex-col h-full">
                            <div className="flex items-center gap-3 mb-4 border-b border-gray-100 pb-2">
                              ...
                              <div>
                                <h3 className="text-lg font-bold text-gray-900 leading-tight">Cadastro Manual</h3>
                                <p className="text-[10px] text-gray-400">Registrar aluno sem gerar contrato agora.</p>
                              </div>
                            </div>
                            ...
                            <div className="space-y-4 overflow-y-auto pr-2 max-h-[400px]">
                              ...
                              <div className="bg-blue-50/50 p-3 rounded-lg border border-blue-100 space-y-3">
                                ...
                                <input autoFocus type="text" value={newStudentName} onChange={(e) => setNewStudentName(e.target.value)} className="w-full p-2 border border-gray-300 rounded-md text-sm bg-white focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Nome Completo" />
                                <input type="text" value={newStudentPhone} onChange={(e) => setNewStudentPhone(e.target.value)} className="w-full p-2 border border-gray-300 rounded-md text-sm bg-white focus:ring-2 focus:ring-blue-500 outline-none" placeholder="WhatsApp (com DDD)" />
                              </div>

                              {/* BLOCO 2: OPCIONAIS (MAS RECOMENDADOS) */}
                              <div className="space-y-3">
                                <label className="text-[10px] font-bold text-gray-400 uppercase border-b border-gray-100 block pb-1">
                                  Dados Complementares (Opcional)
                                </label>

                                <div className="grid grid-cols-2 gap-2">
                                  <input type="text" placeholder="CPF" value={extraData.cpf} onChange={e => setExtraData({ ...extraData, cpf: e.target.value })} className="w-full p-2 border border-gray-200 rounded-md text-sm outline-none focus:border-blue-400" />
                                  <input type="text" placeholder="RG" value={extraData.rg} onChange={e => setExtraData({ ...extraData, rg: e.target.value })} className="w-full p-2 border border-gray-200 rounded-md text-sm outline-none focus:border-blue-400" />
                                </div>

                                <input type="email" placeholder="E-mail do Aluno" value={extraData.email} onChange={e => setExtraData({ ...extraData, email: e.target.value })} className="w-full p-2 border border-gray-200 rounded-md text-sm outline-none focus:border-blue-400" />

                                <input type="text" placeholder="Endereço Completo (Rua, Nº, Bairro, Cidade)" value={extraData.address} onChange={e => setExtraData({ ...extraData, address: e.target.value })} className="w-full p-2 border border-gray-200 rounded-md text-sm outline-none focus:border-blue-400" />

                                <div className="grid grid-cols-2 gap-2">
                                  <div className="space-y-1">
                                    <label className="text-[9px] text-gray-400 font-bold uppercase">Nascimento</label>
                                    <input type="date" value={extraData.birthDate} onChange={e => setExtraData({ ...extraData, birthDate: e.target.value })} className="w-full p-2 border border-gray-200 rounded-md text-sm outline-none text-gray-600 focus:border-blue-400" />
                                  </div>
                                  <div className="space-y-1">
                                    <label className="text-[9px] text-gray-400 font-bold uppercase">Profissão</label>
                                    <input type="text" placeholder="Ex: Advogado" value={extraData.profession} onChange={e => setExtraData({ ...extraData, profession: e.target.value })} className="w-full p-2 border border-gray-200 rounded-md text-sm outline-none focus:border-blue-400" />
                                  </div>
                                </div>
                              </div>
                            </div>

                            <button onClick={handleQuickRegister} className="w-full py-3 bg-black text-white font-bold rounded-lg hover:bg-gray-800 transition-colors shadow-lg mt-4 text-sm flex items-center justify-center gap-2">
                              <Save className="w-4 h-4" /> Salvar Cadastro
                            </button>
                          </div>

                          {/* OPÇÃO B: CONTRATO COMPLETO */}
                          <div className="bg-white p-8 rounded-2xl border-2 border-gray-100 shadow-sm hover:border-black transition-all group cursor-pointer relative" onClick={() => setApprovalStep(1)}>
                            <div className="absolute top-4 right-4 bg-gray-100 text-gray-500 text-[10px] font-bold px-2 py-1 rounded uppercase">
                              Padrão
                            </div>
                            <div className="w-14 h-14 bg-gray-100 text-gray-600 rounded-full flex items-center justify-center mb-6 group-hover:bg-black group-hover:text-white transition-colors">
                              <FileSignature className="w-7 h-7" />
                            </div>
                            <h3 className="text-xl font-bold text-gray-900 mb-2">Cadastro + Contrato</h3>
                            <p className="text-sm text-gray-500 mb-6">Fluxo completo: cadastra o aluno, escolhe o plano, preenche o contrato e envia o link de assinatura.</p>

                            <button className="w-full py-3 border border-gray-200 text-gray-600 font-bold rounded-lg group-hover:bg-black group-hover:text-white group-hover:border-black transition-colors">
                      ...
                      <div className="w-full h-full overflow-y-auto bg-gray-50 p-6">
                        <div className="max-w-4xl mx-auto space-y-6">
                          ...
                          <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm space-y-4">
                            ...
                            {/* --- NOVO TOGGLE: VINCULAR ONBOARDING? --- */}
                            <div className="flex items-center justify-between bg-gray-50 p-3 rounded-lg border border-gray-200">
                              <div>
                                <span className="block text-sm font-bold text-gray-800">Liberar Acesso ao App/Onboarding?</span>
                                <span className="text-xs text-gray-500">Se desligado, o aluno só assina o contrato (Ideal para renovação).</span>
                              </div>
                              <button
                                onClick={() => setIncludeOnboarding(!includeOnboarding)}
                                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${includeOnboarding ? 'bg-green-500' : 'bg-gray-300'}`}
                              >
                                <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${includeOnboarding ? 'translate-x-6' : 'translate-x-1'}`} />
                              </button>
                            </div>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              {/* Só mostra o seletor de plano se o toggle estiver ligado */}
                        ...
                        <tr key={student.id} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="p-4">
                            {student.status === 'signed' ? (
                              ...
                              // --- BOTÃO DE GERAR CONTRATO (NOVO) ---
                              <button onClick={() => openApproveModal(student)} className="inline-flex items-center gap-1 bg-gray-100 text-gray-700 py-1 px-3 rounded-full text-xs font-bold border border-gray-200 hover:bg-black hover:text-white hover:scale-105 transition-all shadow-sm">
                          ...
                          <td className="p-4 text-right flex justify-end gap-2 items-center">
                            {/* BOTÃO DE NOVO CONTRATO / RENOVAÇÃO (PARA TODOS) */}
                            <button
                              onClick={() => openApproveModal(student)}
                              className="p-2 border border-gray-200 rounded-lg text-gray-700 hover:bg-black hover:text-white transition-colors shadow-sm"
                              title="Gerar Novo Contrato ou Renovação"
                            >
                            ...
                            <button
                              ...
                              className="p-2 border border-gray-200 rounded-lg text-gray-500 hover:bg-purple-50 hover:text-purple-600 hover:border-purple-200 transition-colors shadow-sm"
          ...
          <div className="fixed inset-0 bg-black/60 z-[9999] flex items-center justify-center p-4 backdrop-blur-sm">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in duration-200">
              <div className="bg-gray-100 p-4 border-b border-gray-200 flex justify-between items-center">
                ...
                <button onClick={() => setIsHistoryOpen(false)} className="p-2 hover:bg-gray-200 rounded-full"><X className="w-5 h-5 text-gray-500" /></button>
              </div>

              <div className="p-6 max-h-[60vh] overflow-y-auto bg-gray-50">
                {loadingHistory ? (
                  <div className="text-center py-8 text-gray-500"><Loader className="w-8 h-8 animate-spin mx-auto mb-2" />Carregando contratos...</div>
                  ...
                  <div className="space-y-3">
                    {historyContracts.map(contract => (
                      <div key={contract.id} className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm flex items-center justify-between hover:border-black transition-colors">
                        <div>
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            {contract.status === 'signed' ? (
                              <span className="bg-green-100 text-green-700 text-[10px] font-bold px-2 py-0.5 rounded-full border border-green-200">ASSINADO</span>
                            ) : (
                              <span className="bg-yellow-100 text-yellow-700 text-[10px] font-bold px-2 py-0.5 rounded-full border border-yellow-200">PENDENTE</span>
                            )}
                          </div>
                          <p className="text-xs text-gray-400 font-mono">
                            Gerado em: {new Date(contract.createdAt).toLocaleDateString('pt-BR')} às {new Date(contract.createdAt).toLocaleTimeString('pt-BR').slice(0, 5)}
                          </p>
                          <p className="text-[10px] text-gray-300 mt-1">ID: {contract.id}</p>
                        </div>
                        <div className="flex items-center gap-2">
                          ...
                          {contract.contractPdfUrl ? (
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="flex items-center gap-2 px-3 py-1.5 bg-black text-white rounded-lg text-xs font-bold hover:bg-gray-800 transition-transform hover:scale-105 shadow-sm"
                            >
                            ...
                            // CASO 3: Pendente
                            <div className="flex items-center gap-1 text-yellow-600 bg-yellow-50 px-2 py-1 rounded border border-yellow-200">
