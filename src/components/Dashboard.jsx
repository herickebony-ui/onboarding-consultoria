...
import { 
  Users, FileText, Plus, Copy, Settings, Trash2, Edit, 
  Link as LinkIcon, Search, FileSignature, CheckCircle, 
  Loader, X, ChevronRight, Save, Share2, Smartphone, Wallet  // <--- ADICIONE AQUI
} from 'lucide-react';
import { collection, getDocs, setDoc, doc, updateDoc, deleteDoc } from "firebase/firestore";
import { db } from '../services/firebase';
import { generateSlug, applyStudentValuesToContract } from '../utils/helpers';
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
          ...
          <div className="bg-white p-1 rounded-xl border border-gray-200 flex shadow-sm">
            ...
            <button onClick={() => setActiveTab('financial')} className={`px-6 py-2 rounded-lg text-sm font-bold transition-all flex items-center gap-2 ${activeTab === 'financial' ? 'bg-black text-white shadow-md' : 'text-gray-500 hover:bg-gray-50'}`}><Wallet className="w-4 h-4"/> Financeiro</button>
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              ...
              {plans.map((plan) => (
                <div 
                  ...
                  style={{ borderLeft: `6px solid ${plan.color || '#e5e7eb'}`, borderTop: '1px solid #f3f4f6', borderRight: '1px solid #f3f4f6', borderBottom: '1px solid #f3f4f6' }}
                >
                  <div>
                    <div className="flex justify-between items-start mb-2 pl-2">
                      ...
                      <div className="flex items-center gap-1">
                        <div className="relative group/color">
                           <div className="w-6 h-6 rounded-full border border-gray-200 cursor-pointer shadow-sm" style={{ backgroundColor: plan.color || '#ffffff' }} />
                           <input type="color" className="absolute inset-0 opacity-0 cursor-pointer w-full h-full" onChange={(e) => onUpdatePlanColor(plan.id, e.target.value)} value={plan.color || "#ffffff"}/>
                        </div>
                  ...
                  <div className="space-y-2 pl-2">
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => copyLink(plan.id)} className="flex-1 py-2 bg-white border border-gray-300 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-50 flex items-center justify-center gap-2"><LinkIcon className="w-4 h-4"/> Link</button>
          ...
          <div className="animate-in fade-in duration-300">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex flex-col justify-between">
                    <div><p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Total de Alunos</p><h3 className="text-3xl font-black text-gray-900 mt-1">{students.length}</h3></div>
                    <div className="p-2 bg-gray-50 rounded-lg text-gray-400 self-end"><Users className="w-6 h-6"/></div>
                </div>
                <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex flex-col justify-between">
                    <div><p className="text-xs font-bold text-gray-400 uppercase tracking-wider">Novos (Mês)</p><h3 className="text-3xl font-black text-gray-900 mt-1">{students.filter(s => s.createdAt && s.createdAt.startsWith(new Date().toISOString().slice(0, 7))).length}</h3></div>
                    <div className="p-2 bg-blue-50 rounded-lg text-blue-600 self-end"><Plus className="w-6 h-6"/></div>
                </div>
                <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex flex-col justify-between">
                    <div><p className="text-xs font-bold text-yellow-600 uppercase tracking-wider">Falta Assinar</p><h3 className="text-3xl font-black text-yellow-600 mt-1">{students.filter(s => s.status !== 'signed').length}</h3></div>
                    <div className="p-2 bg-yellow-50 rounded-lg text-yellow-600 self-end"><FileSignature className="w-6 h-6"/></div>
                </div>
                <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex flex-col justify-between">
                    <div><p className="text-xs font-bold text-red-600 uppercase tracking-wider">Falta Entregar</p><h3 className="text-3xl font-black text-red-600 mt-1">{students.filter(s => !s.materialDelivered).length}</h3></div>
                    <div className="p-2 bg-red-50 rounded-lg text-red-600 self-end"><CheckCircle className="w-6 h-6"/></div>
                </div>
            </div>

            {/* FILTROS E BUSCA */}
            <div className="flex flex-wrap items-center gap-2 w-full md:w-auto mb-4">
                <div className="relative">
                    <Search className="w-4 h-4 absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400"/>
                    <input type="text" placeholder="Buscar aluno..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-9 p-2 border border-gray-300 rounded-lg text-sm bg-white shadow-sm outline-none focus:border-black w-40"/>
                </div>
                <input type="month" value={filterMonth} onChange={(e) => setFilterMonth(e.target.value)} className="p-2 border border-gray-300 rounded-lg text-sm bg-white shadow-sm outline-none focus:border-black"/>
                <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="p-2 border border-gray-300 rounded-lg text-sm bg-white shadow-sm outline-none focus:border-black font-medium">
              ...
              <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                <div className="bg-white rounded-xl shadow-2xl w-full max-w-6xl h-[90vh] flex flex-col overflow-hidden animate-in zoom-in duration-200">
                  <div className="bg-gray-100 border-b border-gray-300 p-4 flex justify-between items-center">
                    <div>
                        <h3 className="font-bold text-lg text-gray-800 flex items-center gap-2">
                            {approvalStep === 1 ? <><Users className="w-5 h-5"/> Passo 1: Dados & Negociação</> : <><FileSignature className="w-5 h-5"/> Passo 2: Revisão Final da Minuta (Word)</>}
                        </h3>
                    </div>
                    <button onClick={() => setIsInviting(false)} className="p-2 hover:bg-gray-200 rounded-full transition-colors"><X className="w-6 h-6 text-gray-500"/></button>
                  </div>
                        ...
                        <div className="w-full h-full overflow-y-auto bg-gray-50 p-6">
                            <div className="max-w-4xl mx-auto space-y-6">
                                ...
                                <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm space-y-4">
                                    <h4 className="font-bold text-gray-700 text-sm uppercase border-b pb-2 mb-2 flex items-center gap-2"><FileText className="w-4 h-4"/> Dados Cadastrais</h4>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    {editingStudentId && (
                                        <button onClick={handleSaveDataOnly} className="w-full py-3 bg-blue-50 text-blue-600 font-bold rounded-lg hover:bg-blue-100 flex items-center justify-center gap-2 border border-blue-200 mt-4 transition-colors"><Save className="w-4 h-4"/> Salvar Alterações nos Dados</button>
                                    )}
                                </div>
                                <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm space-y-4">
                                    <h4 className="font-bold text-gray-700 text-sm uppercase border-b pb-2 mb-2 flex items-center gap-2"><Settings className="w-4 h-4"/> Configuração do Contrato</h4>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    {selectedTemplateId && (
                                        <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-200 animate-in fade-in mt-4">
                                            <h4 className="text-xs font-bold text-yellow-700 uppercase mb-3">Variáveis de Negociação</h4>
                  ...
                  <div className="bg-white border-t border-gray-300 p-4 flex justify-end gap-3 z-50">
                    ...
                    {approvalStep === 1 ? (
                        <button onClick={handleGenerateDraft} className="px-8 py-3 bg-black text-white rounded-lg font-bold hover:bg-gray-800 shadow-lg flex items-center gap-2">Próximo: Revisar Minuta <ChevronRight className="w-4 h-4"/></button>
                    ) : (
                        <>
                            ...
                            <button onClick={handleFinalizeInvite} className="px-8 py-3 bg-green-600 text-white rounded-lg font-bold hover:bg-green-700 shadow-lg flex items-center gap-2"><CheckCircle className="w-4 h-4"/> Aprovar e Enviar Link</button>
                        </>
                    )}
                  </div>
                </div>
                        ...
                        <tr key={student.id} className="hover:bg-gray-50 transition-colors">
                          ...
                          <td className="p-4">
                            {student.status === 'signed' ? (
                                <span className="inline-flex items-center gap-1 bg-green-100 text-green-700 py-1 px-2 rounded-full text-xs font-bold border border-green-200"><CheckCircle className="w-3 h-3"/> Assinado</span>
                            ) : student.status === 'em_analise' ? (
                                <button onClick={() => openApproveModal(student)} className="inline-flex items-center gap-1 bg-blue-100 text-blue-700 py-1 px-3 rounded-full text-xs font-bold border border-blue-200 hover:bg-blue-200 hover:scale-105 transition-all animate-pulse"><FileSignature className="w-3 h-3"/> Analisar Cadastro</button>
                            ) : (
                                <span className="inline-flex items-center gap-1 bg-yellow-100 text-yellow-700 py-1 px-2 rounded-full text-xs font-bold border border-yellow-200"><Loader className="w-3 h-3"/> Aguardando Assinatura</span>
                            )}
                          </td>
                          <td className="p-4 text-right flex justify-end gap-2 items-center">
                              <button onClick={() => onToggleDelivery(student)} className={`group flex items-center gap-1 px-3 py-1.5 rounded-lg border transition-all ${student.materialDelivered ? 'bg-green-600 border-green-600 text-white shadow-md' : 'bg-white border-gray-300 text-gray-400 hover:border-green-400 hover:text-green-600'}`} title={student.materialDelivered ? "Material já entregue" : "Marcar como entregue"}>
                                  {student.materialDelivered ? <CheckCircle className="w-4 h-4 fill-current" /> : <CheckCircle className="w-4 h-4" />}
                                  <span className="text-[10px] font-bold uppercase hidden xl:inline">{student.materialDelivered ? "Entregue" : "Pendente"}</span>
                              </button>
                              <div className="w-px h-6 bg-gray-200 mx-1"></div>
                              <button onClick={() => openApproveModal(student)} className="p-2 border border-gray-200 rounded-lg text-blue-600 hover:bg-blue-50" title="Editar"><Edit className="w-4 h-4" /></button>
                              {student.status === 'signed' && (
                                  <button onClick={() => generateContractPDF(student)} className="p-2 border border-gray-200 rounded-lg text-green-600 hover:bg-green-50 hover:border-green-200" title="PDF"><FileSignature className="w-4 h-4" /></button>
                              )}
                              <button onClick={() => copyStudentLink(student.id)} className="p-2 border border-gray-200 rounded-lg text-gray-600 hover:bg-blue-50" title="Link"><Share2 className="w-4 h-4" /></button>
                              <button onClick={() => {if(confirm('Remover?')) onDeleteStudent(student.id)}} className="p-2 border border-gray-200 rounded-lg text-gray-400 hover:bg-red-50"><Trash2 className="w-4 h-4" /></button>
                          </td>
                        </tr>
                      ))}                    
                    </tbody>
              ...
              <div className="bg-white rounded-xl shadow-xl border border-gray-200 flex flex-col h-[85vh]">
                <div className="p-4 border-b border-gray-200 flex justify-between items-center bg-gray-50 rounded-t-xl">
                  <div className="flex items-center gap-4 flex-1">
                    <button onClick={() => setIsEditingTemplate(false)}><ChevronRight className="w-5 h-5 text-gray-500 rotate-180"/></button>
                ...
                <div className="flex-1 overflow-hidden h-full flex flex-col md:flex-row">
                  ...
                  <div className="w-full md:w-1/3 p-6 bg-gray-50 overflow-y-auto">
                    ...
                    <div className="bg-blue-50 p-4 rounded-xl border border-blue-200 mb-6 shadow-sm">
                      <h4 className="text-xs font-bold text-blue-800 uppercase mb-2 flex items-center gap-2"><CheckCircle className="w-3 h-3"/> Automáticas</h4>
                      <p className="text-[10px] text-blue-600 mb-3 leading-tight">Preenchimento automático via cadastro.</p>
                      <div className="grid grid-cols-2 gap-2">
                        {['{{nome}}', '{{cpf}}', '{{rg}}', '{{endereco}}', '{{email}}', '{{telefone}}', '{{profissao}}', '{{nascimento}}'].map(tag => (
                          <div key={tag} className="bg-white border border-blue-100 rounded px-2 py-1 text-[10px] font-mono text-blue-700 font-bold text-center cursor-pointer hover:bg-blue-100 transition-colors" onClick={() => {navigator.clipboard.writeText(tag); alert(`Copiado: ${tag}`)}} title="Clique para copiar">{tag}</div>
                        ))}
                      </div>
                    </div>
